        ...
        if(op === '-'){
            ...
            if(selectedSize){
                ...
                if(counter < maxItems){
                    ...
                    setErrors({ ...errors, maxItems: 'You can only buy this number of products'});
                }
            }
        }
    };
                    ...
                    <div className="md:w-full bg-primary/20 mt-4 rounded-2xl flex flex-col p-4">
                        <div className="mt-5 flex justify-between">
                            ...
                            <div className="flex gap-4 text-lg">
                                <span className="bg-primary/50 hover:bg-primary rounded-2xl py-0.5 px-4" onClick={() => handleCounter('-')}>-</span>
                                <span>{counter}</span>
                                <span className="bg-primary/50 hover:bg-primary rounded-2xl py-0.5 px-4" onClick={() => handleCounter('+')}>+</span>
                            </div>
                        </div>
                            ...
                            <div className="flex flex-col gap-2 mx-auto" id='colors'>
                                ...
                                <div className="flex gap-2 mx-auto justify-center">
                                    {item.colours?.map((color, index) => (
                                        <span 
                                            key={index}
                                            className={`w-6 h-6 ${color[0]} rounded-full hover:border cursor-pointer ${color[1] === selectedColor ? 'border-4' : ''}`} 
                            ...
                            <div className="flex flex-col gap-2 mx-auto">
                                ...
                                <div className="flex mx-auto justify-center text-md">
                                    {stock[selectedColor]?.map((size, index) => (
                                        <span 
                                            key={index} 
                                            className={`hover:bg-primary rounded-2xl py-0.5 px-2 cursor-pointer ${size.size === selectedSize ? 'bg-primary' : ''}`} 
